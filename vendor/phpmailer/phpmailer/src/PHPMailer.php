...
class PHPMailer
{
    ...
    protected function edebug($str)
    {
        ...
        if (
            !in_array($this->Debugoutput, ['error_log', 'html', 'echo']) and
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if (
            $this->has8bitChars(substr($address, ++$pos)) and
            static::idnSupported()
        ) {
            if ('Reply-To' != $kind) {
    ...
    public function preSend()
    {
        ...
        if (
            ini_get('mail.add_x_header') == 1 and
            'mail' == $this->Mailer and
            stripos(PHP_OS, 'WIN') === 0 and
            (version_compare(PHP_VERSION, '7.0.0', '>=') and
                version_compare(PHP_VERSION, '7.0.17', '<') or
                version_compare(PHP_VERSION, '7.1.0', '>=') and
        ...
        try {
            ...
            if (
                !empty($this->DKIM_domain) and
                !empty($this->DKIM_selector) and
                (!empty($this->DKIM_private_string) or
                    !empty($this->DKIM_private) and
    ...
    protected static function isShellSafe($string)
    {
        // Future-proof
        if (
            escapeshellcmd($string) !== $string or
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (
            count($this->all_recipients) > count($bad_rcpt) and
    ...
    public function createHeader()
    {
        ...
        if (
            ('sendmail' == $this->Mailer or
                'qmail' == $this->Mailer or
                'mail' == $this->Mailer) and
            count($this->bcc) > 0
        ) {
    ...
    public function encodeHeader($str, $position = 'text')
    {
        ...
        if ($matchcount > strlen($str) / 3) {
            ...
            if ($str == $encoded) {
                ...
                $encoded = trim(
                    chunk_split($str, static::STD_LINE_LENGTH, static::$LE)
                );
            }
            $encoded = str_replace(static::$LE, "\n", trim($encoded));
    ...
    {
        $this->RecipientsQueue = array_filter($this->RecipientsQueue, function (
            $params
        ) use ($kind) {
    ...
    protected function serverHostname()
    {
        ...
        if (!empty($this->Hostname)) {
            ...
            isset($_SERVER) and array_key_exists('SERVER_NAME', $_SERVER)
        ) {
    ...
    public function DKIM_QP($txt)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            $ord = ord($txt[$i]);
            if (
                0x21 <= $ord and $ord <= 0x3a or
                $ord == 0x3c or
                0x3e <= $ord and $ord <= 0x7e
            ) {
                $line .= $txt[$i];
            } else {
    ...
    protected function doCallback(
        ...
        if (
            !empty($this->action_function) and
